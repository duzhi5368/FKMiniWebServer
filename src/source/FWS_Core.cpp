...
void _mwInitSocketData(HttpSocket *phsSocket)
{
	...
	if (!phsSocket->buffer) {
		phsSocket->buffer = (char*)(malloc(HTTP_BUFFER_SIZE));
	}
...
int _mwProcessReadSocket(HttpParam* hp, HttpSocket* phsSocket)
{
		...
		if (iLength <= 0) {
			...
			if (ISFLAGSET(phsSocket, FLAG_REQUEST_POST) && phsSocket->pxMP && hp->pfnFileUpload) {
				...
				if (!pxMP->pchBoundaryValue[0]) {
					...
					(hp->pfnFileUpload)(phsSocket->pxMP, (OCTET*)(phsSocket->pucData), (int)phsSocket->dataLength, hp->pchWebPath);
				}
				_mwCloseSocket(hp, phsSocket);
				return 0;
			}
	...
	if (phsSocket->request.headerSize == 0) { //FIXME
		...
		if (_mwParseHttpHeader(phsSocket)) {
			...
			phsSocket->request.pucPath = (char*)(malloc(pathLen + 1));
			...
			if (ISFLAGSET(phsSocket, FLAG_REQUEST_POST)) {
				if (phsSocket->pxMP) {
					...
					if (ISFLAGSET(phsSocket, FLAG_MULTIPART)) {
						...
						if (hp->pfnFileUpload) (hp->pfnFileUpload)(pxMP, (OCTET*)(phsSocket->buffer + phsSocket->request.headerSize), phsSocket->dataLength, hp->pchWebPath);
					...
					phsSocket->request.pucPayload = (char*)(malloc(phsSocket->bufferSize));
	...
	if (ISFLAGSET(phsSocket, FLAG_REQUEST_POST) && phsSocket->pxMP) {
		if (!phsSocket->pxMP->pchBoundaryValue[0]) {
			...
			if (phsSocket->dataLength == phsSocket->bufferSize && hp->pfnFileUpload) {
				(hp->pfnFileUpload)(phsSocket->pxMP, (OCTET*)(phsSocket->pucData), phsSocket->dataLength, hp->pchWebPath);
				phsSocket->dataLength = 0;
			}
		}
		return 0;
...
void _mwCloseSocket(HttpParam* hp, HttpSocket* phsSocket)
{
	...
	if (phsSocket->pxMP && hp->pfnFileUpload) {
		int i;
		HttpMultipart *pxMP = phsSocket->pxMP;
		(hp->pfnFileUpload)(pxMP, 0, 0, hp->pchWebPath);
	...
	if (phsSocket->fd == INVALID_HANDLE_VALUE) {
		...
		*(p++) = SLASH;
		for (i = 0; i<sizeof(defaultPages) / sizeof(defaultPages[0]); i++) {
			...
			if (phsSocket->fd > 0) {
				fstat(phsSocket->fd, &st);
				hfp.pchExt = (char*)(strchr(defaultPages[i], '.')) + 1;
				break;
			}
#else
			{
...
int _mwProcessMultipartPost(HttpParam *httpParam, HttpSocket* phsSocket, BOOL fNoRecv)
{
	...
	for (; pchBoundarySearch != NULL; pchBoundarySearch = _mwFindMultipartBoundary(phsSocket->buffer,
		...
		if (pxMP->pchFilename[0] && pxMP->oFileuploadStatus != HTTPUPLOAD_LASTCHUNK) {
			...
			pxMP->oFileuploadStatus = HTTPUPLOAD_LASTCHUNK;
			(httpParam->pfnFileUpload)(pxMP, (OCTET*)(phsSocket->buffer), (DWORD)pchBoundarySearch - (DWORD)phsSocket->buffer, httpParam->pchWebPath);
	...
	if (pxMP->writeLocation == HTTPMAXRECVBUFFER) {
		if (pxMP->oFileuploadStatus != HTTPUPLOAD_LASTCHUNK) {
			...
			if ((httpParam->pfnFileUpload)(pxMP, (OCTET*)(phsSocket->buffer), HTTPUPLOAD_CHUNKSIZE, httpParam->pchWebPath)) {
				return 1;
			}
			pxMP->oFileuploadStatus = HTTPUPLOAD_MORECHUNKS;
